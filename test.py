...
def removeStopWords(text):   
    ...
    sentence_without_sw = ' '.join(text_without_sw)
...
with open('/home/shaikh-11201/Desktop/python/Artificial-Intelligence/data.csv') as csv_file:
        ...
        try:
            if line_count == -1:
                ...
                if len(row[0]) < 11:
                    ...
                    frequencySent.append(removeStopWords(row[1]).strip())
                    # print(removeStopWords(row[1]).strip())
                #print(f'\t{row[0]} , {row[1]} ')
                line_count += 1
        except:
            print("")
    ...
    for x in wordList:
        for d in dataDictionaryArray:
            if d["word"] == x:
                ...
                if negate==1:
                    freqJoy = freqJoy + float(d["frequencySadness"] / d["frequency"])
                    freqFear = freqFear + float(d["frequencyFear"] / d["frequency"])
                    freqDanger = freqDanger + float(d["frequencyDanger"] / d["frequency"])
                    freqSadness = freqSadness + float(d["frequencyJoy"] / d["frequency"])
